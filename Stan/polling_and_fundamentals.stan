...
model {
  ...
  for (d in 1:D_f) { 
    ...
    mu_d[1:n_parties_f[d]] = alpha_f[index_f[1:n_parties_f[d], d]] + 
      beta_lag_f * (x_f[index_f[1:n_parties_f[d], d], d] - alpha_f[index_f[1:n_parties_f[d], d]]) + 
...
generated quantities {
  ...
  array[D + pred_y_time_diff, P] int<lower = 0> y_rep;      // Replicated data for all time points
  ...
  for (d in 1:(D + pred_y_time_diff)) {
    ...
    vector[P] pi_d = softmax(eta_d);                       // Convert to probabilities
