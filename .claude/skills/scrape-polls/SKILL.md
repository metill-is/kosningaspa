...
3. **Maskína's own site at `https://maskina.is/frettir/` and the Fylgi-Althingi PDF report.** Maskína publishes a PDF report for every **regular monthly Alþingi reading** at `maskina.is/wp-content/uploads/YYYY/MM/YYYY-MM-Fylgi-Althingi_Maskinuskyrsla.pdf` (sometimes with `-DD` date suffix when multiple). This is the analogue of Gallup's PDF and the authoritative source for `svarendur` and exact field period. Discover the URL via the news listing at `maskina.is/frettir/` (each monthly poll gets its own news article whose body links to the PDF — paginate via `/frettir/page/N/`) or via `WebSearch site:maskina.is "<month> <year>"`.
   ...
   **Important: regular monthly vs commissioned side-publications.** Not every poll that appears under the "Maskína" brand goes through this channel. Maskína occasionally publishes commissioned polls for partner outlets — Vísir, Sýn (Stöð 2), Sýn's Kryddsíldarþáttur, etc. — that **don't** get their own Maskína-authored article or PDF, only the partner outlet's write-up. Examples: the 2025-11-10 reading was for Vísir; the 2026-02-10 one (published 24 Feb) was for Sýn. Both appear on the dashboard as monthly entries but Maskína never published a self-authored article or PDF for them. If `WebSearch site:maskina.is` returns nothing for a known poll date, assume it's a commissioned side-publication. Take the field period and values from the downstream article (source #4), but take `svarendur` from the dashboard sum below when the article doesn't print it, and never estimate n.

   **The definitive PDF list is WordPress's media API**: `curl 'https://maskina.is/wp-json/wp/v2/media?search=Fylgi-Althingi&per_page=100&_fields=source_url,date'`. It lists every Maskínuskýrsla ever uploaded (upload folder ≠ reading month: `2026/07/2026-06-…`). If a reading is not listed there, it is media-only and has no PDF, so stop probing URLs. The PDFs are PowerPoint exports whose `pdftotext` output is fragmented glyph by glyph, so `-layout` and `-raw` are both garbled. Strip **all** whitespace from the plain `pdftotext` output, then regex the joined string: `Könnuninfórframdagana…` / `Súfyrrivarframkvæmddagana…` for the field period, and `Gildsvör([\d.]+)100,0%` for `svarendur`. Table rows read `<Party><count><pct>%±<ci><pct>%`; the repeated pct pins down where the count ends.

   There's also a public dashboard at [`maskina.is/fylgi-flokka-a-althingi/`](https://maskina.is/fylgi-flokka-a-althingi/): a Tableau Public viz (`public.tableau.com/views/FylgiFlokka-heimasa/Njastamling?:embed=y&:showVizHome=no`) covering every Maskína reading back to Nov 2021, including commissioned ones. There is no `.csv` export (`allowDataAccess: false`), **but it is the one source of exact `svarendur` for media-only readings**. Pick the month in `Veldu mælingu (Bakgrunnur)` (the options are DOM elements), then hover each segment of the `Allir` bar with **real** mouse hovers, since Tableau ignores synthetic JS events. Read `document.querySelector('.tab-tooltipContent').innerText` (`Hlutfall` + `Fjöldi`) after each hover. The segments are sorted by share, so each centre is at `x0 + width·(cum% + pct/2)/100`. **Σ `Fjöldi` = `Gild svör` exactly** (exact on the 3 readings with an independently published n, 2026-09-23). Hover the tail for Lýðræðisflokkurinn/Ábyrg framtíð slivers in early 2025. The dashboard's free-text methodology line is **not** reliable: for 2026-08 it says 3.460, while the counts and press both give 3.172.

   **Vísir's Infogram embeds are the fastest discovery and reconciliation channel.** Vísir edits the same two charts in place every month. `e.infogram.com/120df4ad-46bb-41ce-be41-e7f86616dbde` (bar chart, every reading since Apr 2023, labelled by *publication* date) and `e.infogram.com/68399b2f-2841-4338-91fe-7c17c3b3317c` (line chart, one row per month) both carry 1-dp values in `window.infographicData`, which you can pull with `curl`. Diff the bar chart against the tribble on every ingest to catch readings that were never ingested; that diff found 11 on 2026-09-23. It has had one-off slips (2025-08 B 6,3 against the PDF's 6,5), so a PDF or the dashboard wins on conflict.
...
- **Source**: primary URL first, secondary in parentheses if available.
  ...
  - Maskína → `[PDF](maskina.is/wp-content/uploads/...)` when a Maskínuskýrsla exists, else `[Vísir](visir.is/g/...)`; add `[maskina.is dashboard](https://maskina.is/fylgi-flokka-a-althingi/)` in parentheses when n came from the dashboard sum
...
- **Maskína field period unpublished.** Some media-only readings (2025-02, 2025-04, 2025-12, 2026-02) never state a start date anywhere: not in Vísir, RÚV, mbl, the dashboard or Wikipedia. Don't use the publication date, which runs 4–21 days late. The tribble convention (2026-09-23) is **end − 7 d** when only the end date is known, otherwise **publication − 14 d** (Maskína's median lag). Flag the row "date ESTIMATED" in `data_sources.md`. Wikipedia's polling table is a good cross-check on `svarendur` but not on dates.
