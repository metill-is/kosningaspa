...
levels(polling_data$fyrirtaeki)

stan_data$desired_weight <- 0.22
stan_data$weight_time <- 180
stan_data$last_poll_days <- 9
stan_data$last_poll_house <- 7
stan_data$n_last_poll <- 1400
...
model <- cmdstan_model(
  here("Stan", "polling_and_fundamentals_xi.stan")
)
...
fit$summary("beta0") |>
  ...
  select(flokkur, mean, q5, q95, rhat) |>
  arrange(desc(mean))
...
fit$summary("alpha_f") |>
  ...
  select(flokkur, mean, q5, q95, rhat, ess_bulk) |>
  arrange(desc(mean)) |>
  semi_join(
    polling_data,
    by = join_by(flokkur)
...
d_beta |>
  mutate(
    ...
    flokkur = colnames(stan_data$y_n)[p + 1],
    dags = dates[t]
  ) |>
  filter(
...
d_pi |>
  ...
  filter(
    dags >= clock::date_build(2016, 1, 1),
    flokkur %in% c("Annað")
  ) |>
  ggplot(aes(dags, mean)) +
  geom_ribbon(
...
d_yrep |>
  mutate(
    t = str_match(variable, "y_latent\\[(.*),.*\\]")[, 2] |> parse_number(),
    p = str_match(variable, "y_latent\\[.*,(.*)\\]")[, 2] |> parse_number(),
    flokkur = colnames(stan_data$y_n)[p],
    dags = dates[t]
  ) |>
  mutate(mean = median) |>
  ...
  geom_line(aes(col = flokkur), linewidth = 1) +
  geom_point(aes(y = konnun, col = flokkur), size = 2)
...
p <- plot_dat |>
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Spáð fylgi flokka í könnun Maskínu 21. nóvember",
    subtitle = str_c(
...
polling_data |>
  ...
  filter(flokkur == "Miðflokkurinn", n > 0)
