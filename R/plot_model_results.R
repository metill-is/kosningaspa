...
d <- read_parquet(here("data", "2024-11-28", "y_rep_draws_constituency.parquet")) |>
summarise(
  mean = median(value),
...
coverage_data <- read_parquet(here("data", "2024-11-28", "y_rep_draws_constituency.parquet"))  |>
reframe(
...
p3 <- d |>
filter(dags >= clock::date_build(2016, 1, 1)) |>
ggplot(aes(dags, mean, colour = litur, data_id = flokkur)) +
geom_vline(
  xintercept = clock::date_build(2024, 11, 30),
  alpha = 0.4
